...
export function detectSourceMapLeaks(files, owner, repo, branch) {
    const leaks = [];
    for (const file of files) {
        if (!SOURCE_MAP_EXTENSIONS.some((ext) => file.endsWith(ext)))
            continue;
        const parts = file.toLowerCase().split("/");
        if (parts.some((p) => SOURCE_MAP_IGNORE_DIRS.includes(p)))
            continue;
        leaks.push({
            file,
...
export function detectHomoglyphs(text, context) {
    const issues = [];
